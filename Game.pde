...
void draw() {
  ...
  for (GameObject playerPart : playerOne.getParts()) {
    playerPart.show();
    if (wKey) {
      playerPart.accelerateangle(speedPlayer, 270);
    }
    if (aKey) {
      playerPart.accelerateangle(speedPlayer, 180);
    }
    if (sKey) {
      playerPart.accelerateangle(speedPlayer, 90);
    }
    if (dKey) {
      playerPart.accelerateangle(speedPlayer, 0);
    }
